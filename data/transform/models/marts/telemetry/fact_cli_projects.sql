SELECT
    ...
    SUM(CASE WHEN is_plugin_singer THEN event_count ELSE 0 END) AS singer_event_total,
    SUM(CASE WHEN is_plugin_airflow THEN event_count ELSE 0 END) AS airflow_event_total,
    SUM(CASE WHEN is_plugin_dagster THEN event_count ELSE 0 END) AS dagster_event_total,
    SUM(
        CASE WHEN is_plugin_lightdash THEN event_count ELSE 0 END
    ) AS lightdash_event_total,
    SUM(CASE WHEN is_plugin_superset THEN event_count ELSE 0 END) AS superset_event_total,
    SUM(CASE WHEN is_plugin_sqlfluff THEN event_count ELSE 0 END) AS sqlfluff_event_total,
    SUM(
        CASE WHEN is_os_feature_environments THEN event_count ELSE 0 END
    ) AS environments_event_total,
    SUM(CASE WHEN is_os_feature_test THEN event_count ELSE 0 END) AS test_event_total,
    SUM(CASE WHEN is_os_feature_run THEN event_count ELSE 0 END) AS run_event_total,
    COALESCE(SUM(
        event_count
