...
executions as (
    SELECT
        ...
        -- Structured
        max(base.se_category) as struct_comand_category,
...
SELECT
    -- TODO: why are so many commands null? Are we parsing them wrong? Are they not coming through?
...
INNER JOIN
    {{ ref('event_src_activation') }} ON
        executions.project_id = event_src_activation.project_id
WHERE
    executions.started_ts >= event_src_activation.sp_activate_date
